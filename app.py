...
def index():
    ...
    return render_template('index.html',trending_products=trending_products.head(8),truncate = truncate,  #trending_products  hmra pass rating based recommandation se bnya huwa final ans data ha
...
def indexredirect():
    ...
    random_product_image_urls = [random.choice(random_image_urls) for _ in range(len(trending_products))]
    price = [40, 50, 60, 70, 100, 122, 106, 50, 30, 50]
    return render_template('index.html', trending_products=trending_products.head(8), truncate=truncate,  #yha pe trending_products=trending_products.head(8) ka sath wo cheesa he jis ki madad se hum ne kuch funtionality krwani thi trending_products=trending_products.head(8) pe
                           random_product_image_urls=random_product_image_urls,
                           random_price=random.choice(price))
...
def recommendations():
    if request.method == 'POST':
        ...
        print(f"Product: {prod}, Number: {nbr}")
        ...
        if content_based_rec.empty:
            ...
            print(f"Random Product Image URLs: {random_product_image_urls}")
